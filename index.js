...
function processSum(numberList, callback3) {
  return callback3(numberList.reduce(function(accumulator, num){return accumulator + num;},0));
}
...
function getFullNames(runners){
  ...
  runners.forEach((person) => {
    return fullName.push(`${person.last_name}, ${person.first_name}`);
  });
  return fullName;
 }
...
function firstNamesAllCaps(runners) {
  let cap = runners.map(function(person){
...
*/
function getRunnersByTShirtSize(runners, T_ShirtSize) {
  let shirtSizes = runners.filter(person => {return person.shirt_size === T_ShirtSize;})
  return (shirtSizes);
}
...
function tallyUpDonations(runners) {
  let totalDonations = runners.reduce((accumulator, num) => {return (accumulator + num.donation);}, 0);
